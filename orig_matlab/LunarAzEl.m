...
%function [Az h] = LunarAzEl(UTC,Lat,Lon,Alt)
% 18 1 2013 JG, make UTC a vector of matlab times
% 18 1 2013 JG vectorize it
...
UTC=UTC(:)';
jd = juliandate(UTC); % 18 1 2013 jg, make it matlab time
...
Obl = Obl.*(pi/180);
Nobs=length(Obl);
for i=1:Nobs, % 21 1 2013 JG need to loop over all angles
RotM = [1 0 0; 0 cos(Obl(i)) sin(Obl(i)); 0 -sin(Obl(i)) cos(Obl(i))]';
...
sol(:,i) = RotM*[xeclip(i) yeclip(i) zeclip(i)]'.*EarthRadEq;
end
...
theta1=180-acosd([sol(1,:)-xsl;sol(2,:)-ysl;sol(3,:)-zsl]'*[xsl;ysl;zsl] ...
...
theta2 = 180 - acosd([sol(1,:)-xel; sol(2,:)-yel;sol(3,:)-zel]'*[xel;yel;zel] ...
...
hourvec = datevec(UTC); % 18 1 2013, JG make it matlab time
...
id=sin(HA.*(pi/180)) >= 0; %nk 19.01.214 verctorized 
Az(id) = 360-Az(id);
...
Az=reshape(Az,sizeA,sizeB);
h=reshape(h,sizeA,sizeB);
